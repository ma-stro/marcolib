...
"
  (error  "HEY, ~a , WHAT A CURIOUS SYNTHI YOU WANNA ME TO PLAY ...!~%COME ON, ~a!~%       in function ~a"
          (getenv 'USER) synth function))
   ...
   ((stringp name)    
      (let* ((wt-dir
             (make-pathname :directory (append
                                           (pathname-directory (getenv 'LLwt))
                                           subdirs)))
...
specified by the environment variable LLwt"
	...
	(format t "~%HAPPY TO SERVE YOU, ~a~%" (getenv 'ENV-USER))
