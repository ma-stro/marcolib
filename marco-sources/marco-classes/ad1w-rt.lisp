  ...
  (
   ( source-code :initform
                 (load-buffer-textfile
                  (get-orc-source (get-orc "ad1w"))
                  'textfile "append")
                 :allocation :class
                 :type textfile
                 :accessor source-code)
   ( numchan :initform (or (get-orc-channels (get-orc "ad1w")) 1)
...
;=============================================================================
;		AD1w.ORC
; ADDITIVE SYNTHESIS INSTRUMENT N. 11 / COMPLETE / TRIAPHONIC (no ch. 4)
;=============================================================================
...
(defclass! ad1w
  ...
  (:documentation
   "
Extension of the instrument ADD1s, class ad1w-rt.
...
(defmethod get-slot-in-out-names ((self ad1w))
...
(defmethod fixed-slots-list ((x om::ad1w))
