  ...
  (
   ( source-code :initform
                 (load-buffer-textfile
                  (get-orc-source (get-orc "ad1s" :local) )
                  'textfile "append")
                 :allocation :class
                 :type textfile
                 :accessor source-code)
   ( numchan :initform (or (get-orc-channels (get-orc "ad1s" :local)) 1)
   ...
   ( macros-list :initform 
                 (get-orc-macros (get-orc "ad1s" :local))
                 :allocation :class
                 :type list
...
;  5)
(defmethod add-n-slots ((self om::ad1s)) -5)
;******************************************************************
